...
//    MPI_Type_commit(&plane_XY);
   ...
   if(x_high!=MPI_PROC_NULL){
       MPI_Isend(local_data,1,plane_YZ_send_high,x_high,0,MPI_COMM_WORLD,&send_request_x[1]);
       MPI_Irecv(local_data,1,plane_YZ_recv_high,x_high,1,MPI_COMM_WORLD,&recv_request_x[1]);
   }
   if(x_low!=MPI_PROC_NULL){
       MPI_Irecv(local_data,1,plane_YZ_recv_low,x_low,0,MPI_COMM_WORLD,&recv_request_x[0]);
       MPI_Isend(local_data,1,plane_YZ_send_low,x_low,1,MPI_COMM_WORLD,&send_request_x[0]);
   }
   if(y_high!=MPI_PROC_NULL){
       MPI_Isend(local_data,1,plane_XZ_send_high,y_high,0,MPI_COMM_WORLD,&send_request_y[1]);
       MPI_Irecv(local_data,1,plane_XZ_recv_high,y_high,1,MPI_COMM_WORLD,&recv_request_y[1]);
   }
   if(y_low!=MPI_PROC_NULL){
       MPI_Irecv(local_data,1,plane_XZ_recv_low,y_low,0,MPI_COMM_WORLD,&recv_request_y[0]);
       MPI_Isend(local_data,1,plane_XZ_send_low,y_low,1,MPI_COMM_WORLD,&send_request_y[0]);
   }
   if(z_high!=MPI_PROC_NULL){
       MPI_Isend(local_data,1,plane_XY_send_high,z_high,0,MPI_COMM_WORLD,&send_request_z[1]);
       MPI_Irecv(local_data,1,plane_XY_recv_high,z_high,1,MPI_COMM_WORLD,&recv_request_z[1]);
   }
   if(z_low!=MPI_PROC_NULL){
       MPI_Irecv(local_data,1,plane_XY_recv_low,z_low,0,MPI_COMM_WORLD,&recv_request_z[0]);
       MPI_Isend(local_data,1,plane_XY_send_low,z_low,1,MPI_COMM_WORLD,&send_request_z[0]);
   }
//    if(x_high!=MPI_PROC_NULL){
