...
// **Allocate buffers for packing**
    ...
    MPI_Pack_size(lx * ly * lz * nc, MPI_FLOAT, MPI_COMM_WORLD, &buffer_size);
    buffer_size += MPI_BSEND_OVERHEAD;

    for (int i = 0; i < size; i++) {
        Pro[i] = malloc(buffer_size);
        positions[i] = 0;  // Initialize positions
    }
    // if (rank == 0) { // Root process: Send data
//         for (int x = 0; x < nx; x++) {
//             for (int y = 0; y < ny; y++) {
//                 for (int z = 0; z < nz; z++) {
...
//                         int buffer_size = nc * sizeof(float);
//                         char buffer[buffer_size];
//                         int position = 0;

//                         // Pack data into buffer
//                         MPI_Pack(data[x][y][z], nc, MPI_FLOAT, buffer, buffer_size, &position, MPI_COMM_WORLD);

//                         // Send packed buffer
//                         MPI_Send(buffer, position, MPI_PACKED, process, (x % lx) * ly * lz + (y % ly) * lz + (z % lz), MPI_COMM_WORLD);
//                     } else {
//                         // Store locally
//                         for (int c = 0; c < nc; c++) 
//                             local_data[x+1][y+1][z+1][c] = data[x][y][z][c];
//                     }
//                 }
//             }
//         }
...
//                     // Receive packed buffer
//                     MPI_Recv(buffer, buffer_size, MPI_PACKED, 0, x * ly * lz + y * lz + z, MPI_COMM_WORLD, &status);

//                     // Unpack data
//                     MPI_Unpack(buffer, buffer_size, &position, local_data[x+1][y+1][z+1], nc, MPI_FLOAT, MPI_COMM_WORLD);
//                 }
//             }
//         }
//     }
...
float *recv_buffer = malloc(lx * ly * lz * nc * sizeof(float));
if (myrank == 0) {
...
MPI_Scatter(send_buffer, lx * ly * lz * nc, MPI_FLOAT,
                recv_buffer, lx * ly * lz * nc, MPI_FLOAT, 0, MPI_COMM_WORLD);
int idx = 0;
    for (int x = 0; x < lx; x++) {
        for (int y = 0; y < ly; y++) {
            for (int z = 0; z < lz; z++) {
                for (int c = 0; c < nc; c++) {
                    local_data[x + 1][y + 1][z + 1][c] = recv_buffer[idx++];
                }
            }
        }
    }
